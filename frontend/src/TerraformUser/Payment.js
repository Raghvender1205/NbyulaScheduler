...
const Payment = (props) => {
    ...
    function createEvent(id, dateTime, terraformEmail) {
        var virtualEvent = {
            id: id,
            summary: "Appointment",
            location: "Virtual",
            description: "Terraform-TerraformUser appointment",
            start: {
                dateTime: dateTime,
            ...
            attendees: [{ email: terraformEmail }],
    ...
    const { dateId, terraform, slotId } = props.location.data;

    const bookSlot = async () => {
        const { data } = await Axios.post(
            `${process.env.REACT_APP_SERVER_URL}/terraform/book-slot/`,
            {
                googleId: localStorage.getItem("googleId"),
                terraformUserName: JSON.parse(localStorage.getItem("user")).name,
                slotId: slotId,
                dateId: dateId,
                doctorId: terraform._id,
            }
        );
    ...
    useEffect(() => {
        setFinalBalnce(1.18 * terraform.feesPerSession);
    }, [terraform.feesPerSession]);

    const makePayment = async (token) => {
        const { data } = await Axios.post(
            `${process.env.REACT_APP_SERVER_URL}/terraformUser/payment`,
            {
                token,
                finalBalnce,
            }
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td className="text-center">
                                                        <p>
                                                            <strong>{terraform.feesPerSession}</strong>
                                                        </p>
                                                        <p>
                                                            <strong>{0.18 * terraform.feesPerSession}</strong>
                                                        </p>
                                                    </td>
                                                </tr>
                                                <tr>
