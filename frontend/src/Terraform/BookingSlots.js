...
import LeftsideTerraformUser from "../Dashboard/LeftsideTerraformUser";

import Axios from "axios";

const BookingSlots = (props) => {
    // console.log(props.location.state)
    const { date, terraform } = props.location.state;
    ...
    useEffect(() => {
        const fetchDate = async (dateToPost) => {
            const { data } = await Axios.post(
                `${process.env.REACT_APP_SERVER_URL}/terraform/get-slots/`,
                {
                    terraformId: terraform._id,
                    date: dateToPost
                }
            );
            console.log(data);
                                    ...
                                    <tr key={slot._id}>
                                        ...
                                        {slot.isBooked ? (
                                            ...
                                            <td>
                                                <Link
                                                    to={{
                                                        pathname: "/terraformUser/payment",
                                                        data: {
                                                            dateId: dateId,
                                                            doctor: terraform,
                                                            slotId: slot._id,
